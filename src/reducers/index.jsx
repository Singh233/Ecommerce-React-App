  ...
  switch (action.type) {
    case ADD_NEW_PRODUCT:
      ...
      if (lsProducts !== null) {
        ...
        localStorage.setItem(
          "products",
          JSON.stringify([action.product, ...lsProducts])
        );
      } else {
        // add to local storage
      ...
      if (allProducts !== null) {
        ...
        localStorage.setItem(
          "allProducts",
          JSON.stringify([action.product, ...allProducts])
        );
      } else {
        // add to local storage
    ...
    case DELETE_PRODUCT:
      ...
      if (productsFromLS.length > 0) {
        const newProductsFromLocalStorage = productsFromLS.filter(
          (product) => product.id !== action.id
        );
        localStorage.setItem(
          "products",
          JSON.stringify(newProductsFromLocalStorage)
        );
      }
    ...
    case UPDATE_PRODUCT:
      ...
      const productsFromLocalStorage3 = JSON.parse(
        localStorage.getItem("products")
      );
      if (productsFromLocalStorage3.length > 0) {
        const newProductsFromLocalStorage = productsFromLocalStorage3.map(
          (product) => {
    ...
    case SORT_BY_LATEST:
      const productsFromLocalStorage4 = JSON.parse(
        localStorage.getItem("products")
      );
      let products2 = [];
    ...
    case SORT_BY_ELECTRONICS:
      ...
      const electronics = allProductsLocalStorage.filter(
    ...
    case SORT_BY_HOME_AND_KITCHEN:
      ...
      const allProductsLocalStorage2 = JSON.parse(
        localStorage.getItem("allProducts")
      );
      const homeAndKitchen = allProductsLocalStorage2.filter(
    ...
    case SORT_BY_CATEGORY_ALL:
      const allProductsLocalStorage3 = JSON.parse(
        localStorage.getItem("allProducts")
      );
      return {
        ...state,
  ...
  switch (action.type) {
    case ADD_TO_CART:
        ...
        localStorage.setItem(
          ...
          JSON.stringify(
            parseInt(localStorage.getItem("totalPrice")) +
              Math.round(parseFloat(action.product.price))
          )
        );
      } else {
        localStorage.setItem(
          "totalPrice",
          JSON.stringify(Math.round(parseFloat(action.product.price)))
        );
      }
      ...
      return {
        ...
        totalPrice:
          state.totalPrice + Math.round(parseFloat(action.product.price)),
      };
    case REMOVE_FROM_CART:
      ...
      // remove from state
      const cart = cartFromLocalStorage.filter(
        (product) => product.id !== action.product.id
      );
      if (cart.length !== 0) {
